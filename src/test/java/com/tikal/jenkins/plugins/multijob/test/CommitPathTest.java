...
public class CommitPathTest {
	...
	public void testSkippingJobBasedOnChangelog()
		...
		FreeStyleProject free1 = jenkinsRule.jenkins.createProject(FreeStyleProject.class, "job1");
		free1.getBuildersList().add(new Shell("echo hello from job-1"));
		FreeStyleProject free2 = jenkinsRule.jenkins.createProject(FreeStyleProject.class, "job2");
		free2.getBuildersList().add(new Shell("echo hello from job-2"));
		List<PhaseJobsConfig> configList = new ArrayList<>();
		configList.add(new PhaseJobsConfig("job1", null, true, null, KillPhaseOnJobResultCondition.NEVER, false, false, "", 0, false, false, "", false, false, true, "beer/mikkeller/.+"));
	...
	public void testChangelogWithAdditionalCommitShallRunAllJobs()
		...
		free2.getBuildersList().add(new Shell("echo hello from job-2"));
		List<PhaseJobsConfig> configList = new ArrayList<>();
		configList.add(new PhaseJobsConfig("job1", null, true, null, KillPhaseOnJobResultCondition.NEVER, false, false, "", 0, false, false, "", false, false, true, "beer/mikeller/.+"));
		configList.add(new PhaseJobsConfig("job2", null, true, null, KillPhaseOnJobResultCondition.NEVER, false, false, "", 0, false, false, "", false, false, false, ""));
		multi.getBuildersList().add(new MultiJobBuilder("Phase", configList, ContinuationCondition.SUCCESSFUL, MultiJobBuilder.ExecutionType.PARALLEL));
		multi.setScm(scm);
		mockEmptyChangeSet();
		jenkinsRule.assertBuildStatus(Result.SUCCESS, multi.scheduleBuild2(0, new UserCause()).get());
		mockChangeSet("beer/mikkeller/AmericanDream", "whisky/bruichladdich/Octomore");
		jenkinsRule.assertBuildStatus(Result.SUCCESS, multi.scheduleBuild2(0, new UserCause()).get());
		assertLogContains("MultiJob should run job1", multi, "Starting build job job1.");
		assertLogContains("MultiJob should run job2", multi, "Starting build job job2.");
		assertLogContains("shell task writes 'hello from job-1' to log", free1, "hello from job-1");
		assertLogContains("shell task writes 'hello from job-2' to log", free2, "hello from job-2");
		assertEquals("there should be one phase and three projects", 4, multi.getView().getRootItem(multi).size());
	}
